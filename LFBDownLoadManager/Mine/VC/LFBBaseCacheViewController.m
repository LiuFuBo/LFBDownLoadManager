...
#import "LFBBaseCacheViewController.h"
...
@interface LFBBaseCacheViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation LFBBaseCacheViewController
...
- (void)creatBaseControl
{
    ...
    tabbarView.backgroundColor = [UIColor colorFromString:@"#fafafa"];
    ...
    for (int i = 0; i < titleArray.count; i++) {
        ...
        [btn setBackgroundImage:[UIImage imageWithColor:[UIColor colorFromString:@"#2f9cd4"]] forState:UIControlStateHighlighted];
        [btn setTitleColor:[UIColor colorFromString:@"#2f9ad2"] forState:UIControlStateNormal];
    ...
    for (int i = 0; i < 2; i++) {
        ...
        line.backgroundColor = [UIColor colorFromString:@"#e5e5e5"];
...
{
    LFBHomeTableViewCell *cell = [_tableView cellForRowAtIndexPath:[NSIndexPath indexPathForRow:index inSection:0]];
...
//        self->_tabbarView.frame = CGRectMake(0, tabbarY, KMainW, self->_allSelbtn.height + KBottomSafeArea);
//        self->_tableView.height += (btn.selected ? - tableViewChangeH : tableViewChangeH);
    }];
}
...
- (void)tabbarBtnOnClick:(UIButton *)btn
{
    ...
    if (btn == _allSelbtn) {
        ...
        for (NSIndexPath *indexPath in indexPaths) {
            ...
            [[LFBDownLoadManager shareManager] deleteTaskAndCache:self.dataSource[indexPath.row]];
        }
        ...
        dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.2f * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
            [self navBtnOnClick:self->_navRightBtn];
            [self reloadNavRightBtn];
        });
    }
}
